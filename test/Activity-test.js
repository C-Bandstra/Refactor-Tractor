import { expect } from 'chai'
import Activity from '../src/Activity';
import UserRepository from '../src/UserRepository';
import User from '../src/User';

describe('Activity', function() {
  let user;
  let current1;
  let current2;
  let activity1;
  let activity2;
  let data;

  beforeEach(() => {

  user = new User({
    'id': 1,
    'name': 'Luisa Hane',
    'address': '15195 Nakia Tunnel, Erdmanport VA 19901-1697',
    'email': 'Diana.Hayes1@hotmail.com',
    'strideLength': 4.3,
    'dailyStepGoal': 10000,
    'friends': [
      1,
      2,
      3
    ]
  })

  current1 = {
    "userID": 1,
    "date": "2019/06/15",
    "numSteps": 3577,
    "minutesActive": 140,
    "flightsOfStairs": 16
  }

  current2 = {
    "userID": 1,
    "date": "2019/06/16",
    "numSteps": 11000,
    "minutesActive": 175,
    "flightsOfStairs": 36
  }

  activity1 = new Activity(user, current1);
  activity2 = new Activity(user, current2);

  });

  it('should be a function', function() {
    expect(Activity).to.be.a('function');
  });
  it('should be an instance of activity', function() {
    expect(activity1).to.be.an.instanceof(Activity);
  });
  it('should hold a userId', function() {
    expect(activity1.user.id).to.equal(1);
  });
  it('should hold a date', function() {
    expect(activity1.date).to.equal("2019/06/15");
  });
  it('should hold number of steps', function() {
    expect(activity1.steps).to.equal(3577);
  });
  it('should hold minutes active', function() {
    expect(activity1.minutesActive).to.equal(140);
  });
  it('should hold flights of stairs', function() {
    expect(activity1.flightsOfStairs).to.equal(16);
  });
  it('should have a default value of 0 for miles walked', function() {
    expect(activity1.milesWalked).to.equal(0);
  });
  it('should have a default value of null for reached step goal', function() {
    expect(activity1.reachedStepGoal).to.equal(null);
  });
  it('doActivity should add activities to user record', function() {
    // invokes do activity upon instantiation
    expect(user.activityRecord.length).to.equal(2);
  });
  it('should have a method that calculate miles walked', function() {
    expect(activity1.calculateMiles()).to.equal('3.0');
  });
  describe('compareStepGoal', function() {
    it('should return false if goal isn\'t met', function() {
      activity1.compareStepGoal();
      expect(activity1.reachedStepGoal).to.equal(false);
    });
    it('should return true if goal is met', function() {
      activity2.compareStepGoal();
      expect(activity2.reachedStepGoal).to.equal(true);
    });
  });
});
